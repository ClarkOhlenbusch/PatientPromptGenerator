...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    // Use memory store as fallback if no pool is available
    const MemoryStore = createMemoryStore(session);
    if (!pool) {
      console.warn('No PostgreSQL pool available, using memory store for sessions');
    ...
    try {
      ...
      // We already created the MemoryStore wrapper above
      this.sessionStore = new MemoryStore({
        checkPeriod: 86400000 // prune expired entries every 24h
      });
    }
