...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/sandbox/variables/:id", async (req, res) => {
    try {
      ...
      const updatedVariable = await storage.updateTemplateVariable(parseInt(id), updates);
  ...
  app.post("/api/regenerate-prompt-with-system-and-variables", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ success: false, message: "Authentication required" });
      }
      ...
      console.error("Error regenerating prompt with system and variables:", err);
      return res.status(500).json({
        success: false,
